...
export function buildGrid(raw) {
  ...
  // Aggregate counts and injured counts by month x hour
  const counts = d3.rollup(raw, v => v.length, d => timeMonth.floor(d.date), d => d.hour);
  const injCounts = d3.rollup(raw, v => d3.sum(v, r => r.injured ? 1 : 0), d => timeMonth.floor(d.date), d => d.hour);
  ...
  for (const month of months) {
    for (const h of hours) {
      ...
      const a = injCounts.get(month)?.get(h) || 0;
      const rate = c > 0 ? a / c : 0;
      grid.push({ month, hour: h, count: c, injured: a, rate });
